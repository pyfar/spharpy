...
class SamplingSphere(Coordinates):
    ...
    def spherical_voronoi(self, round_decimals=13, center=0.0):
        ...
        """
        points = self.cartesian.T
        radius = np.unique(np.round(self.radius, decimals=round_decimals))
        if len(radius) > 1:
    ...
    def calculate_sampling_weights(self, round_decimals=12, apply=False):
        ...
        """
        sv = self.spherical_voronoi(round_decimals=round_decimals)
        ...
        area = np.zeros(self.n_points, np.double)
