...
def test_sphharm_init():
    ...
    sph_harm = SphericalHarmonics(n_sh=2, coordinates=coordinates)
    assert sph_harm.n_sh == 2
    assert np.all(sph_harm.coordinates == coordinates)

def test_sphharm_init_invalid_coordinates():
    with pytest.raises(TypeError):
        SphericalHarmonics(n_sh=2, coordinates=[0, 0, 1])

def test_sphharm_init_invalid_n_sh():
    coordinates = pf.Coordinates(1, 0, 0)
    with pytest.raises(ValueError):
        SphericalHarmonics(n_sh=-1, coordinates=coordinates)
...
def test_compute_basis_caching():
    ...
    sh = SphericalHarmonics(n_sh, coordinates)
